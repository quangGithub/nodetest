...
RealEstate.getRealEstateByID = (id, result) => {
    dbConn.query('SELECT * FROM realestate WHERE RealEstateId = ?', id, (err, res) => {
        if (err) {
...
RealEstate.createRealEstate = (realEstateReqData, result) => {
    dbConn.query('INSERT INTO realestate SET ?', realEstateReqData, (err, res) => {
        if (err) {
...
RealEstate.updateRealEstate = (RealEstateId, realEstateReqData, result) => {
    dbConn.query("UPDATE realestate SET Title=?,RoomCode=?, Price=?, Location=?, Address=?, Direct=?, Floor=?, imgURL=?, Area=?, Toilet=?, People=?, Detail=?, Building=?, CreateAt=?, UserId=? WHERE RealEstateId= ?",[realEstateReqData.Title, realEstateReqData.RoomCode, realEstateReqData.Price, realEstateReqData.Location, realEstateReqData.Address, realEstateReqData.Direct, realEstateReqData.Floor, realEstateReqData.imgURL,
        realEstateReqData.Area, realEstateReqData.Toilet, realEstateReqData.People, realEstateReqData.Detail, realEstateReqData.Building, realEstateReqData.CreateAt, realEstateReqData.UserId, RealEstateId], (err, res) => {
        if (err) {
...
RealEstate.deleteRealEstate = (id, result)=> {
    dbConn.query('DELETE FROM realestate WHERE RealEstateId=?', [id], (err, res)=>{
        if(err){
