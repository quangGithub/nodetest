...
User.updateUser = (UserId, userReqData, result) => {
    dbConn.query("UPDATE user SET Username=?, Email=?, Avatar=?, ethAddress=?, CreateAt=?, isAdmin=? WHERE UserId= ?",[userReqData.Username, userReqData.Email, userReqData.Avatar, userReqData.ethAddress, userReqData.CreateAt, userReqData.isAdmin, UserId], (err, res) => {
        if (err) {
